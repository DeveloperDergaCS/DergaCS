*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZEFATT_MAP_TRASV
*   generation date: 29.06.2020 at 16:55:45 by user DEVELOPER
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZEFATT_MAP_TRASV    .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZEFATT_MAP_TRASV'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZEFATT_MAP_TRASV_EXTRACT
TOTAL = ZEFATT_MAP_TRASV_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZEFATT_MAP_TRASV'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZEFATT_MAP_TRASV-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZEFATT_MAP_TRASV_EXTRACT
TOTAL = ZEFATT_MAP_TRASV_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZEFATT_MAP_TRASV-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZEFATT_MAP_TRASV'
...
TOTAL = ZEFATT_MAP_TRASV_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZEFATT_MAP_TRASV-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZEFATT_MAP_TRASV-UPD_FLAG NE SPACE.
...
CALL FUNCTION 'VIEWPROC_ZEFATT_MAP_TRASV'
                    ...
                    IMPORTING
                              UPDATE_REQUIRED =
STATUS_ZEFATT_MAP_TRASV-UPD_FLAG
                    TABLES
                        EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZEFATT_MAP_TRASV_EXTRACT
TOTAL = ZEFATT_MAP_TRASV_TOTAL
